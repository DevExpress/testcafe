...
class RequestMock extends RequestHook {
    ...
    onRequestTo (requestFilterRuleInit) {
        if (this.pendingRequestFilterRuleInit)
            throw new APIError('onRequestTo', RuntimeErrors.requestHookConfigureAPIError, RequestMock.name, "The 'respond' method was not called after 'onRequestTo'. You must call the 'respond' method to provide the mocked response.");
    ...
    respond (body, statusCode, headers) {
        if (!this.pendingRequestFilterRuleInit)
            throw new APIError('respond', RuntimeErrors.requestHookConfigureAPIError, RequestMock.name, "The 'onRequestTo' method was not called before 'respond'. You must call the 'onRequestTo' method to provide the URL requests to which are mocked.");
