...
export default class Runner extends EventEmitter {
    ...
    _validateSpeedOption () {
        ...
        if (typeof speed !== 'number' || isNaN(speed) || speed < 0.01 || speed > 1)
            throw new GeneralError(RuntimeErrors.invalidSpeedValue);
    }
    ...
    _validateConcurrencyOption () {
        ...
        if (typeof concurrency !== 'number' || isNaN(concurrency) || concurrency < 1)
            throw new GeneralError(RuntimeErrors.invalidConcurrencyFactor);
    }
    ...
    _validateScreenshotOptions () {
        ...
        if (!screenshotPath && screenshotPathPattern)
            throw new GeneralError(RuntimeErrors.cantUseScreenshotPathPatternWithoutBaseScreenshotPathSpecified);
    }
    ...
    async _validateVideoOptions () {
        ...
        if (!videoPath) {
            if (videoOptions || videoEncodingOptions)
                throw new GeneralError(RuntimeErrors.cannotSetVideoOptionsWithoutBaseVideoPathSpecified);
        ...
        if (!videoOptions.ffmpegPath)
            throw new GeneralError(RuntimeErrors.cannotFindFFMPEG);
    }
    ...
    _validateScreenshotPath (screenshotPath, pathType) {
        ...
        if (forbiddenCharsList.length)
            throw new GeneralError(RuntimeErrors.forbiddenCharatersInScreenshotPath, screenshotPath, pathType, renderForbiddenCharsList(forbiddenCharsList));
    }
    ...
    src (...sources) {
        if (this.apiMethodWasCalled.src)
            throw new GeneralError(RuntimeErrors.multipleAPIMethodCallForbidden, OPTION_NAMES.src);
    ...
    browsers (...browsers) {
        if (this.apiMethodWasCalled.browsers)
            throw new GeneralError(RuntimeErrors.multipleAPIMethodCallForbidden, OPTION_NAMES.browsers);
    ...
    reporter (name, output) {
        if (this.apiMethodWasCalled.reporter)
            throw new GeneralError(RuntimeErrors.multipleAPIMethodCallForbidden, OPTION_NAMES.reporter);
