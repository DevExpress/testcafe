...
export default class TestRun extends EventEmitter {
    ...
    _getPayloadScript () {
        ...
        return Mustache.render(TEST_RUN_TEMPLATE, {
            ...
            hasMarionette:                !!this.marionetteClient
        });
    }
    ...
    async [CLIENT_MESSAGES.performActions] (msg) {
        if (this.lastMarionetteId && this.lastMarionetteId === msg.id)
            return;

        this.lastMarionetteId = msg.id;

        if (!this.marionetteClient)
            throw new Error('No marionette client');
