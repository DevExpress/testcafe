...
export default class TestRun extends AsyncEventEmitter {
    ...
    public async start (): Promise<void> {
        ...
        await this._executeCommand(new serviceCommands.TestDoneCommand());
    ...
    private async _enqueueSetBreakpointCommand (callsite: CallsiteRecord | undefined, error?: string): Promise<void> {
        ...
        this.debugging = await this._executeCommand(new serviceCommands.SetBreakpointCommand(!!error, inCompilerService), callsite) as boolean;
    }
    ...
    private async _executeAssertion (command: AssertionCommand, callsite: CallsiteRecord): Promise<void> {
        ...
        executor.once('start-assertion-retries', (timeout: number) => this._executeCommand(new serviceCommands.ShowAssertionRetriesStatusCommand(timeout)));
        executor.once('end-assertion-retries', (success: boolean) => this._executeCommand(new serviceCommands.HideAssertionRetriesStatusCommand(success)));
    ...
    public async _adjustCommandOptions (command: CommandBase): Promise<void> {
        ...
        if (command.type === COMMAND_TYPE.typeText) {
            const result = await this._executeCommand((command as any).selector);
            ...
            const result = await this._executeCommand(new serviceCommands.GetActiveElementCommand());
    ...
    public async _executeAction (apiActionName: string, command: CommandBase, callsite: CallsiteRecord): Promise<unknown> {
        ...
        try {
            result = await this._executeCommand(command, callsite);
        }
        catch (err) {
    ...
    public async _executeCommand (command: CommandBase, callsite?: CallsiteRecord | string): Promise<unknown> {
    ...
    public async _makeScreenshotOnFail (): Promise<void> {
        ...
        if (!this.errScreenshotPath && (screenshots as ScreenshotOptionValue)?.takeOnFails)
            this.errScreenshotPath = await this._executeCommand(new browserManipulationCommands.TakeScreenshotOnFailCommand()) as string;
    }
    ...
    public async getStateSnapshot (): Promise<StateSnapshot> {
        ...
        state.storages = await this._executeCommand(new serviceCommands.BackupStoragesCommand()) as StoragesSnapshot;
    ...
    public async switchToCleanRun (url: string): Promise<void> {
        ...
        if (this.speed !== this.opts.speed) {
            ...
            await this._executeCommand(setSpeedCommand);
        }
        ...
        if (this.pageLoadTimeout !== this.opts.pageLoadTimeout) {
            ...
            await this._executeCommand(setPageLoadTimeoutCommand);
        }
        ...
        if (this.activeDialogHandler) {
            ...
            await this._executeCommand(removeDialogHandlerCommand);
        }
    }
    ...
    public async navigateToUrl (url: string, forceReload: boolean, stateSnapshot?: string): Promise<void> {
        ...
        await this._executeCommand(navigateCommand);
    }
    ...
    private async _switchToWindowByPredicate (command: SwitchToWindowByPredicateCommand): Promise<void> {
        const currentWindows = await this._executeCommand(new GetCurrentWindowsCommand({}, this) as CommandBase) as OpenedWindowInformation[];
        ...
        await this._executeCommand(new SwitchToWindowCommand({ windowId: windows[0].id }, this) as CommandBase);
    }
