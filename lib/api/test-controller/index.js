...
var TestController = function () {
    ...
    TestController.prototype._enqueueCommand = function _enqueueCommand(apiMethodName, CmdCtor, cmdArgs) {
        ...
        return this._enqueueTask(apiMethodName, function (callsite) {
            ...
            if (_this2.testRun.recordScreenCapture && apiMethodName !== 'takeScreenshot') {
                return function () {
                    return _this2.testRun.executeCommand(command, callsite).then((0, _asyncToGenerator3.default)( /*#__PURE__*/_regenerator2.default.mark(function _callee() {
                        var date, dateFolder, sessionId, testName, path;
                            ...
                            while (1) {
                                switch (_context.prev = _context.next) {
                                    case 0:
                                        ...
                                        date = new Date();
                                        dateFolder = date.getMonth() + 1 + '_' + date.getDate() + '_' + date.getFullYear();
                                        sessionId = _this2.testRun.session.id;
                                        testName = _this2.testRun.test.name;
                                        path = dateFolder + '/' + sessionId + '/' + Date.now() + '_' + testName + '_' + apiMethodName + '_auto.png';
                                        _context.next = 8;
                                        return _this2.testRun.executeCommand(new _browserManipulation.TakeScreenshotCommand({ path: path }));
