...
TestCafeClient.define('Automation.Util', function (require, exports) {
    var ContentEditableHelper = require('Base.ContentEditableHelper'),
        CursorWidget = require('UI.Cursor'),
...
TestCafeClient.define('Base.ContentEditableHelper', function (require, exports) {
    var Hammerhead = HammerheadClient.get('Hammerhead'),
        $ = Hammerhead.$,
        Util = Hammerhead.Util;
...
TestCafeClient.define('Base.PageState', function (require, exports) {
    var TextSelection = require('Base.TextSelection'),
    ...
    function getInnerHTML(el) {
        var $testCafeRoot = ShadowUI.getRoot();

        if (el.tagName.toLowerCase() === 'body' && hasRootElement(el, $testCafeRoot)) {
            var $cloneBody = $(el).clone(),
                $cloneRoot = ShadowUI.select('#' + $testCafeRoot[0].id, $cloneBody);
    ...
    function hasRootElement(el, $root) {
        var $testCafeRoot = $root || ShadowUI.getRoot();

        return ShadowUI.select('#' + $testCafeRoot[0].id, $(el)).length;
    }
...
TestCafeClient.define('Base.TextSelection', function (require, exports) {
    var ContentEditableHelper = require('Base.ContentEditableHelper'),
...
TestCafeClient.define('Base.Transport', function (require, exports) {
    ...
    //Const
    var WORKER_HEARTBEAT_INTERVAL = 10 * 1000;

    //Exports
    //-------------------------------------------------------------------------------------
    exports.startWorkerHeartbeat = function () {
        var opt = {
            cache: false,
            type:  'GET',
    ...
    exports.switchToStartRecordingUrl = function (recordingUrl) {
        ...
        if (window.location.hash && replacedLocationHref === replacedRecordingUrl &&
            removeHashRegExp.test(recordingUrl))
...
TestCafeClient.define('TestRunner.TestIterator', function (require) {
    ...
    TestIterator.prototype._runStep = function () {
        ...
        if (testIterator.state.step > 0 && !testIterator.state.stepDoneCalled &&
            typeof testIterator.state.stepDone === 'function') {
    ...
    exports.select = function () {
        ...
        if (args.length === 1 && (Util.isDomElement(secondArg) || Util.isTextNode(secondArg))) {
            if (Util.isNotVisibleNode(secondArg))
                ...
                if (!Util.isContentEditableElement(options.startNode) ||
                    !Util.isContentEditableElement(options.endNode))
                    error = true;
                else {
...
(function () {
    ...
    Errors.hasErrorStepName = function (err) {
        return err.code !== Errors.UNCAUGHT_JS_ERROR && err.code !== Errors.XHR_REQUEST_TIMEOUT &&
            err.code !== Errors.IFRAME_LOADING_TIMEOUT;
    };
