...
`quarantineMode`  | Boolean | Defines whether or not to enable the [quarantine mode](../../../guides/basic-guides/run-tests.md#quarantine-mode).                                                                                                                    | `false`
...
`debugOnFail`     | Boolean | Specifies whether or not to enter the debug mode when a test fails. If enabled, the test is paused at the moment it fails, so that you can explore the tested page to determine what caused the failure. | `false`
...
`speed`           | Number  | Specifies the test execution speed. A number between `1` (fastest) and `0.01` (slowest). If individual action speed is also specified, the action speed setting overrides the test speed. | `1`
