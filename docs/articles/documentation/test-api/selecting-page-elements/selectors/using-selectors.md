...
DOM node or the timeout exceeds. If TestCafe cannot find the corresponding node in the DOM, the test fails. The test report shows a sequence of the DOM node's methods that were called during the test and highlights the failed method. This helps you understand what caused the fail.

![DOM node methods in reports](dom-node-methods-in-reports.png)
