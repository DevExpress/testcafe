...
Use the [timeout](../../reference/test-api/selector/constructor.md#optionstimeout) option to specify the timout for a particular selector in test code. 

```js
await t.expect(Selector('#elementId', { timeout: 500 }).innerText).eql('text', 'check element text');
```

To set the timeout for all selectors at once, pass it to the [runner.run](../../reference/testcafe-api/runner/run.md) API method or the [--selector-timeout](../../reference/command-line-interface.md#--selector-timeout-ms) command line option when you launch tests. This will override the timeout option described above.
