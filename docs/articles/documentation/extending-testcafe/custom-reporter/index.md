...
* [Generating Reporter Project](#generating-reporter-project)
* [Implementing Reporter](#implementing-reporter)
* [Building Reporter](#building-reporter)
* [Testing Reporter](#testing-reporter)
* [Previewing Report](#previewing-report)
* [Using Reporter Development Version](#using-reporter-development-version)
* [Publishing Reporter to NPM](#publishing-reporter-to-npm)
* [Reference](#reference)
...
**Note:** It is recommended to name the directory as you would like to name the reporter project. When you run the generator, it will automatically suggest the reporter name that matches the reporter directory name. As for the reporter package name, it will consist of two parts - the `{testcafe-reporter-}` prefix and the name of the reporter itself:
...
You have to implement four reporter methods.
...
These methods should output the desired information at certain moments during test run.

* **reportTaskStart** - fires when a test task is started, that is at the beginning of testing.

* **reportFixtureStart** - fires each time a fixture is started.

* **reportTestDone** - fires each time a test is finished.

* **reportTaskDone** - fires when the entire task is finished, that is at the end of testing.

All the required data is provided to these methods through their parameters.

To output information, use [helper methods](helper-methods.md) and libraries.
...
The following example demonstrates how you can implement four main methods,
so that a generated report contains information about user agents used for testing,
a result of individual test execution, summary information about passed/failed tests,
and overall duration of tests.
...
This is a sample report generated by this reporter:
...
You can build a reporter project by using the `build` Gulp task.
...
To test your reporter operates well, you can use the `test` Gulp task.
To get this task to work, you need to provide reference reports in your format.
...
Then, you have to compose two reference reports.
...
You can preview a report built by your reporter using the `preview` Gulp task.

```bash
gulp preview
```

## Using Reporter Development Version
...
This allows you to work on the reporter project and test it iteratively without having to re-publish everytime you make a change.
