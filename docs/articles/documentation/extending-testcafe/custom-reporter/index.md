...
**Note:** It is recommended to name the directory the same as you would like to name your reporter project. When you run the generator, it will automatically suggest the reporter name that matches the reporter directory name. As for the reporter package name, it will consist of two parts - the `{testcafe-reporter-}` prefix and the name of the reporter itself:
...
In the `src/index.js` file, you can also enable or disable coloring of the reporter output by using the `noColors` property. To color the output, use the [chalk](helper-methods.md#chalk) helper method.
...
* Refer to the test input data at `<reporter-directory>/test/data/test-calls.js`. This file contains a sequence of reporter method calls. Each method uses a number of input data.
* Explore an existing reporter, say, the [list](https://github.com/DevExpress/testcafe-reporter-list) reporter. You can either run the `preview` Gulp task to view the reporter output in your terminal, or find the reference report at `/test/data/report-without-colors`.

Then, compose two reference reports.

* A regular report. Put it into `/test/data/` and name it `report-without-colors`.
* A colored report, which is a JSON file that consists of a single string - the report text with additional color information specified using [ANSI escape codes](https://en.wikipedia.org/wiki/ANSI_escape_code). Put this JSON file into `/test/data/` and name it `report-with-colors.json`.
...
When you finish developing the reporter, you can publish it to npm by using the `publish` Gulp task.
This task also performs additional actions before publishing the package - builds the package and tests if the reporter works correctly.
That is why it is recommended to use this Gulp task instead of `npm publish`.
