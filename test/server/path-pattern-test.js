...
describe('Screenshot path pattern', () => {
    ...
    it('Increment file indexes', () => {
        const pathPattern = createPathPattern();

        expect(pathPattern.data.fileIndex).eql(1);
        expect(pathPattern.data.errorFileIndex).eql(1);

        pathPattern.incrementFileIndexes(false);

        expect(pathPattern.data.fileIndex).eql(2);

        pathPattern.incrementFileIndexes(true);

        expect(pathPattern.data.errorFileIndex).eql(2);
    });
