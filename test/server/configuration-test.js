...
describe('TypeScriptConfiguration', function () {
    ...
    describe('Custom Testcafe Config Path', () => {
        ...
        it('Custom ts config is not well-formed', async () => {
            ...
            fs.writeFileSync(customConfigFile, 'const a: string = 1;');
            configuration = new TestCafeConfiguration(customConfigFile);
            try {
                await configuration.init();
            }
            ...
            const filePath = path.resolve(customConfigFile).replace(/\\/g, '/');

            expect(message).to.equal(
                `Cannot prepare tests due to the following error:\n\n` +
                `Error: TypeScript compilation failed.\n` +
                `${filePath} (1, 7): Type 'number' is not assignable to type 'string'.\n`
            );
        });
