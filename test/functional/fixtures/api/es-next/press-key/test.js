...
describe('[API] t.pressKey', function () {
    it('Should press keys', function () {
        return runTests('./testcafe-fixtures/press-key-test.js', 'Press keys', { only: 'chrome', skip: 'chrome-osx' });
    });
    ...
    it('Should raise event in different windows if focus was changed during action execution', function () {
        return runTests('./testcafe-fixtures/press-key-test.js', 'Press key in iframe', { only: 'chrome', skip: 'chrome-osx' });
    });
});
