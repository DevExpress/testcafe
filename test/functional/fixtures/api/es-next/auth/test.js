...
describe('Basic and NTLM authentications', function () {
    ...
    describe('Using external proxy', function () {
        it('Should not authenticate on a "basic" server with wrong credentials', function () {
            return runTests('./testcafe-fixtures/basic-auth-with-wrong-credentials-test.js', null, { proxy: TRANSPARENT_PROXY_URL });
        });

        it('Should authenticate on a "basic" server with correct credentials', function () {
            return runTests('./testcafe-fixtures/basic-auth-with-correct-credentials-test.js', null, { proxy: TRANSPARENT_PROXY_URL });
        });

        it('Should authenticate on a "NTLM" server with correct credentials', function () {
            return runTests('./testcafe-fixtures/ntlm-auth-check-username-test.js', null, { proxy: TRANSPARENT_PROXY_URL });
        });

        it('Should override credentials with test.httpAuth', function () {
            return runTests('./testcafe-fixtures/override-credentials-test.js', null, { proxy: TRANSPARENT_PROXY_URL });
        });
    });
});
