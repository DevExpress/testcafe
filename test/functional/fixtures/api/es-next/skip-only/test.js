...
describe('[API] .skip/.only', function () {
    it('Should filter out tests without "only" directive', function () {
        return runTests('./testcafe-fixtures/only-test.js', null, { shouldFail: true, only: 'chrome', skip: 'chrome-osx' })
            .catch(function (errs) {
    ...
    it('Should filter out tests without "skip" directive', function () {
        return runTests('./testcafe-fixtures/skip-test.js', null, { shouldFail: true, only: 'chrome', skip: 'chrome-osx' })
            .catch(function (errs) {
