...
describe('[API] Wait', function () {
    describe('t.wait', function () {
        it('Should wait for the specified time', function () {
            return runTests('./testcafe-fixtures/wait-test.js', 'Wait', { only: 'chrome', skip: 'chrome-osx' });
        });

        it('Should validate the timeout argument', function () {
            return runTests('./testcafe-fixtures/wait-test.js', 'Incorrect timeout argument (wait)', { only: 'chrome', skip: 'chrome-osx' })
                .catch(function (errs) {
