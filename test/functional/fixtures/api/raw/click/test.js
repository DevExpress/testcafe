...
describe('[Raw API] Click action', () => {
    ...
    it('Should fail if an action target is invisible', () => {
        return runTests('./testcafe-fixtures/click.testcafe', 'Click invisible button', { shouldFail: true })
            .catch(errs => {
                expect(errs[0]).contains('The action target (<input type="button" id="invisible-button" value="invisible-button" style="display: none;">) is invisible. The value of its \'display\' property is \'none\'.');
    ...
    it('Should fail if an action target is out of the visible area', () => {
        return runTests('./testcafe-fixtures/click.testcafe', 'Click a button that is out of the visible area', { shouldFail: true })
            .catch(errs => {
                expect(errs[0]).contains('The action target (<input type="button" id="out-of-visible-area" style="position: absolute; left: -100px; top: -100px;">) is located outside the the layout viewport.');
