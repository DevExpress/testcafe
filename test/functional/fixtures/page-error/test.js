    ...
    it('Should fail if the error is not caught in the test', function () {
        return runTests('./testcafe-fixtures/page-error-test.js', 'Do not handle', { shouldFail: true })
            .catch(function (errs) {
                ...
                const expectedErrorMessage = config.proxyless
                    ? 'Error: Can only get response body on requests captured after headers received.'
                    : 'Failed to find a DNS-record for the resource at "http://some-unreachable.url/"';

                expect(errs[0]).contains(expectedErrorMessage);
