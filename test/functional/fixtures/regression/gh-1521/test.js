...
describe('[Regression](GH-1521)', function () {
    it('Should wait for a timeout if an element is not in the viewport temporary', function () {
        return runTests('testcafe-fixtures/index-test.js', 'Wait for an out-of-viewport element', { only: 'chrome', skip: 'chrome-osx' });
    });

    it('Should reevaluate selector during waiting for element since it can be changed', function () {
        return runTests('testcafe-fixtures/index-test.js', 'Click on a changing element', { only: 'chrome', skip: 'chrome-osx' });
    });
    ...
    it('Should wait for a timeout if an element is overlapped', function () {
        return runTests('testcafe-fixtures/index-test.js', 'Wait until element is not overlapped', { only: 'chrome', skip: 'chrome-osx' });
    });

    it('Should click on an overlapping element after some timeout', function () {
        return runTests('testcafe-fixtures/index-test.js', 'Click on an overlapping element after some timeout', { only: 'chrome', skip: 'chrome-osx' });
    });

    it('Should wait while element is moving', function () {
        return runTests('testcafe-fixtures/index-test.js', 'Click on a moving element', { only: 'chrome', skip: 'chrome-osx' });
    });
    ...
    it('Should not wait if the offset position of a fixed element is changing', function () {
        return runTests('testcafe-fixtures/index-test.js', 'Click on a fixed element', { only: 'chrome', skip: 'chrome-osx' });
    });

    it('Should raise mouseover/mousemove if hovered element was overlapped', function () {
        return runTests('testcafe-fixtures/index-test.js', 'Hover to an overlapped element', { only: 'chrome', skip: 'chrome-osx' });
    });
});
