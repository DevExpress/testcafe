...
TestCafe v3.5.0 includes multiple enhancements and bug fixes. Pass Selector queries to the Visual Selector Debugger, explore new ways to specify screenshot path patterns, and use a new _experimental_ flag to run multi-window tests with native automation!
...
fixture`Fixture.disableConcurrency`
    .page`https://devexpress.github.io/testcafe/example/`.disableConcurrency;
```
...
TestCafe v3.4.0 _does not_ relaunch unresponsive browsers if you enter [development mode](xref:402638#developmentmode).
...
fixture`TestController.browser`.page`https://example.com`;
...
TestCafe v2.5.0 introduced an _experimental_ mode that allows users to automate Chromium-based browsers, such as Google Chrome and Microsoft Edge, with the native CDP protocol. TestCafe v3.0.0 and up enables this capability out of the box.
...
function onBeforeWriteHook(writeInfo) {
    // This function will fire every time the reporter calls the "write" method.
    if (writeInfo.initiator === "reportTestDone") {
        // The "initiator" property contains the name of the reporter event that triggered the hook.
        const { name, testRunInfo, meta } = writeInfo.data || {}; // If you attached this hook to a compatible reporter (such as "spec" or "list"), the hook can process data related to the event.
        const testDuration = testRunInfo.durationMs; // Save the duration of the test.
        writeInfo.formattedText =
            writeInfo.formattedText + " (" + testDuration + "ms)"; // Add test duration to the reporter output.
    }
}
...
```js
await t.report("text", { key: "value" }, ["arrayItem1", "arrayItem2"]);
```
...
TestCafe v2.3.0 introduces `create-testcafe` --- an interactive tool that allows you to initialize a new TestCafe project in seconds. The update also includes _experimental_ ECMAScript module support and a number of bug fixes.
...
Use the [create-testcafe](https://github.com/devexpress/create-testcafe) tool to initialize a new TestCafe project, or _add_ TestCafe to an existing Node.js application.
...
1. Create a new folder for the TestCafe project _(optional)_.
...
5. Populate the test folder with test examples _(optional)_.
6. Create a YAML file with a GitLab Actions workflow that runs TestCafe tests _(optional)_.
...
TestCafe has always used _CommonJS_ syntax for module imports:
...
TestCafe v2.2.0 introduces user-defined custom actions and an important _experimental_ capability. Google Chrome users can now enable "proxyless mode" to speed up their test suite.
...
```js
fixture`Test structure`.page`./example`; // starts at https://devexpress.github.io/testcafe/example
```
...
```ts
fixture("[API] Delete Cookies").page(
...
test("Should delete all the cookies with the specified url", async (t) => {
    ...
    await t
        ...
        .eql(2)
        .expect(cookies[0])
        .contains({ name: "apiCookie1", path: "/testcafe/example/" })
        .expect(cookies[1])
        .contains({
            name: "apiCookie2",
            path: "/testcafe/example/thank-you.html",
        });
...
Many TestCafe users employ test hooks --- functions that run before and after tests and fixtures. In TestCafe v1.19.0 and higher, you can attach hooks to _test runs_, as well as apply test hooks to your _entire suite_. This capability requires the use of a JavaScript configuration file.
...
fixture`Test user variables`.page(userVariables.url);
...
```js
await t.dispatchEvent("#button", "touchstart", {
...
fixture`Scroll Action`.page("http://example.com");
...
fixture`Scroll Actions`.page`http://www.example.com/`;
...
```js
fixture`Setting Timeouts`.page`http://devexpress.github.io/testcafe/example`;
...
fixture`Target Shadow DOM elements`.page(
    "https://devexpress.github.io/testcafe/example"
);

test("Get text within shadow tree", async (t) => {
    const shadowRoot = Selector("div")
        .withAttribute("id", "shadow-host")
        .shadowRoot();
    const paragraph = shadowRoot.child("p");

    await t
        .expect(paragraph.textContent)
        .eql("This paragraph is in the shadow tree");
});
```
...
fixture`Login page`.page("https://login.wrike.com/login/");
...
test("Login via Google", async (t) => {
    await t
        ...
        .typeText(
            "input[type=email]",
            "This text will be entered inside the pop-up"
        );
});
```
...
fixture`Test page`.page("https://devexpress.github.io/testcafe/example/");
...
test("My test", async (t) => {
    if (t.browser.name !== "Chrome")
        await t
            .expect(Selector("div").withText("Browser not supported").visible)
            .ok();
});
```
...
fixture`My fixture`.page`https://example.com`.beforeEach(async (t) => {
    if (t.browser.engine.name === "Blink") return;
    // ...
});
```
...
test("My test", async (t) => {
    ...
    await t.takeElementScreenshot(
        loginButton,
        `auth/${t.browser.name}/login-button.png`
    );
});
```
...
> TestCafe v1.6.0 also supports the existing options to maintain backward compatibility. However, these options are now marked _obsolete_ in the documentation. In the future updates, we will deprecate them and emit warnings.
...
```js
fixture`My fixture`.page`http://example.com`.clientScripts(
    "mockDate.js",
    "scripts/react-helpers.js"
);
```
...
```js
fixture`My fixture`.clientScripts({
...
-   pass the [path to a JavaScript file](https://devexpress.github.io/testcafe/documentation/using-testcafe/common-concepts/inject-scripts-into-tested-pages.html#inject-a-javascript-file) to inject its content:
    ...
    ```js
    fixture`My fixture`.clientScripts({ path: "assets/jquery.js" });
    ```
...
-   use the [module name](https://devexpress.github.io/testcafe/documentation/using-testcafe/common-concepts/inject-scripts-into-tested-pages.html#inject-a-module) to inject a module:
    ...
    ```js
    fixture`My fixture`.clientScripts({ module: "async" });
    ```
...
-   pass the [code](https://devexpress.github.io/testcafe/documentation/using-testcafe/common-concepts/inject-scripts-into-tested-pages.html#inject-script-code) you need to inject:
    ...
    fixture`My fixture`.clientScripts({
        content:
            "Geolocation.prototype.getCurrentPosition = () => new Position(0, 0);",
    });
    ```
...
Previous versions performed _test syntax validation_ within input script files before executing them. Only files that contained the [fixture](https://devexpress.github.io/testcafe/documentation/test-api/test-code-structure.html#fixtures) and [test](https://devexpress.github.io/testcafe/documentation/test-api/test-code-structure.html#tests) directives were executed.
...
createTestCafe("localhost", 1337, 1338)
    .then((tc) => {
        ...
        return liveRunner.src("tests/test.js").browsers("chrome").run();
    })
    .then(() => {
...
runner.filter((testName, fixtureName, fixturePath, testMeta, fixtureMeta) => {
    return testMeta.mobile === "true" && fixtureMeta.env === "staging";
});
```
...
runner.screenshots({
    ...
    takeOnFails: true,
    pathPattern:
        "${TEST_INDEX}/${OS}/${BROWSER}-v${BROWSER_VERSION}/${FILE_INDEX}.png",
});
```
...
runner.useProxy("172.0.10.10:8080", [
    "localhost:8080",
    "internal-resource.corp.mycompany.com",
]);
```
...
```js
await t
    .expect(doSomethingAsync())
    .ok("check that a promise is returned", { allowUnawaitedPromise: true });
```
...
test("Take a screenshot of a fieldset", async (t) => {
    await t
        ...
        .takeElementScreenshot(
            Selector("fieldset").nth(1),
            "my-fixture/important-features.png"
        );
});
```
...
test("Filter visible and hidden elements", async (t) => {
    ...
    await t
        .expect(hiddenInput.count)
        .eql(1)
        .expect(visibleInputs.count)
        .eql(11);
});
```
...
The current selector's [withText](https://devexpress.github.io/testcafe/documentation/test-api/selecting-page-elements/selectors/functional-style-selectors.html#withtext) method looks for elements whose text content _contains_ the specified string. With this release, we have added the [withExactText](https://devexpress.github.io/testcafe/documentation/test-api/selecting-page-elements/selectors/functional-style-selectors.html#withexacttext) method that searches by _strict match_.
...
test("Search by exact text", async (t) => {
    ...
    await t.expect(winLabel.exists).ok().expect(reusingLabel.exists).ok();
});
```
...
```js
fixture`Page load timeout`.page`http://devexpress.github.io/testcafe/example/`;
...
fixture`react example`.page`http://localhost:8080/` // https://github.com/mzabriskie/react-example
...
test("My Test", async (t) => {
    await t.dragToElement(fileIcon, directoryPane, {
        offsetX: 10,
        offsetY: 10,
...
Enable _concurrency_ and TestCafe launches multiple instances of each browser. It distributes the test batch among them. The tests are run in parallel.
...
fixture`TODO list test`.page("http://localhost:1337");
...
test("Check list item", async (t) => {
    ...
    await t
        .expect(todoItem.getVue().props.priority)
        .eql("High")
        .expect(todoItem.getVue().state.isActive)
        .eql(false)
        .expect(todoItem.getVue().computed.text)
        .eql("Item 1");
});
```
...
A piece of logic that logs in a particular user is called a _role_. It is a good practice to create a role for each user account participating in your test.
...
fixture`My Fixture`.page`http://example.com`;
...
```js
fixture`MyFixture`.page`file:///user/my-website/index.html`;
```

```js
fixture`MyFixture`.page`../my-project/index.html`;
```
...
Use the `ctx` parameter passed to `fixture.before` and `fixture.after` methods (_fixture context_) to share values and objects with test code.
...
fixture`Test Speed`.page`http://devexpress.github.io/testcafe/example/`;
...
```js
fixture`My fixture`.page`http://example.com`.httpAuth({
    username: "username",
    password: "Pa$$word",
...
```js
runner.startApp("node server.js").run();
```
...
```js
runner.startApp("node server.js", 4000).run();
```
...
test("Check Label HTML", async (t) => {
    ...
    await t
        ...
        .contains('input type="checkbox" name="remote"');
});
```
...
test.page`http://devexpress.github.io/testcafe/blog/`("Test2", async (t) => {
...
by using the _test context_ object.
...
const snapshot = await Selector("ul")
    .find("label")
    .parent("div.someClass")
    .nth(4)();
```
